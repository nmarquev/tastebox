...
export const RecipeCard = ({ recipe, onView, onEdit, onDelete, onToggleFavorite, onToggleCooked, onPlayTTS, onShowNutrition, onSaveToCollection, onDropImage, isAddingImage = false, isInCollection = false, columns = 3, collectionNames = [], dishTypeOptions = [], categoryOptions = [], tagOptions = [], sourceOptions = [], allCollections = [], onInlineSave, onToggleFeature, isPlayingTTS = false, isGeneratingScript = false, selectionMode = false, isSelected = false, onSelectionChange }: RecipeCardProps) => {
  ...
  const activeCardFeatures = [
    ...
    recipe.checked ? (
      <span key="checked" title="Chequeada" className="inline-flex h-7 w-7 items-center justify-center text-[#2cbc63]">
  ...
  return (
    <Card className={`group relative flex h-full w-full min-w-0 max-w-full overflow-hidden bg-gradient-card shadow-recipe-card transition-all duration-300 hover:shadow-elegant hover:-translate-y-1 ${oneCol ? "flex-col sm:flex-row" : "flex-col"} ${
      ...
      >
        ...
        <div className={`absolute flex items-center justify-end gap-1.5 ${columns === 3 ? 'left-1/2 top-3 -translate-x-1/2' : compact ? 'right-2 top-2 gap-1 [&_button]:h-6 [&_button]:w-6 [&_svg:not(.recipe-checked-icon)]:!h-3.5 [&_svg:not(.recipe-checked-icon)]:!w-3.5 [&_img]:!h-4 [&_img]:!w-4' : 'right-3 top-3'}`}>
          {onToggleFavorite && (
            <Button
              variant="secondary"
              size="sm"
          ...
          {onToggleFeature && (
            <Button
              ...
              className={`order-2 h-8 w-8 border p-0 ${
                recipe.checked
                  ? 'border-white/60 bg-white/50 text-[#2cbc63] hover:bg-white/70'
            ...
            >
              <Check className="recipe-checked-icon" style={{ width: 18, height: 18 }} strokeWidth={4} />
            </Button>
          )}
          {onSaveToCollection && (
            <Button
